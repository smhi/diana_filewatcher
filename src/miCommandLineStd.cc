/*
  ...
  $Id: miCommandLineStd.cc 4328 2013-12-18 12:59:28Z juergens $
...
miCommandLineStd::miCommandLineStd(const vector<option>& o,
...
char miCommandLineStd::aliasToFlag(const string& s) const
{
...
bool miCommandLineStd::flagLegal(const char c) const
{
  for (unsigned int i=0; i<opts.size(); i++)
    if (opts[i].flag==c)
      return true;
...
bool miCommandLineStd::hasArg(const char c) const
{
  for (unsigned int i=0; i<opts.size(); i++)
    if (opts[i].flag==c)
...
void miCommandLineStd::illegalOptionError(const char* prog, const char opt)
{
  err=true;
...
void miCommandLineStd::requiresArgumentError(const char* prog, const char opt)
{
  err=true;
